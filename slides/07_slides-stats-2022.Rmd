...
The test is sometimes called the Mantel–Cox test, named after Nathan Mantel and David Cox. 

The logrank test can also be viewed as a time-stratified Cochran–Mantel–Haenszel test.
...
(here a death) at any time point. The analysis is based on the times 
of events (here deaths). For each such time we calculate the observed
number of deaths in each group and the number expected if there were 
in reality no difference between the groups.



We can now use a $\chi ^{2}$ test of the null hypothesis. The test statistic is 
the sum of $\sum_{it}$ $\frac{(O_{it} - E_{it})^{2}}{E_{it}}$ for each group, where O and E are the totals of
 the observed and expected events at the time of each event. 
...
```{r logrankpairwise2, echo = TRUE, fig.show = "hide"}
 ...
 pairwise_survdiff(OS_objs ~  smoking, data = tcganonasmoking, p.adjust.method = "none",   rho = 0)
